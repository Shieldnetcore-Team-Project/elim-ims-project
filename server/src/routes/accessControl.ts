...
accessControlRouter.get('/:userId', (req, res) => {
  res.json({ userId: req.params.userId, pages: accessControl.getAccess(req.params.userId) });
});

accessControlRouter.put('/:userId', safe((req, res) => {
  const { pages, actor } = req.body ?? {};
  if (!Array.isArray(pages)) { res.status(400).json({ error: '"pages" must be an array of page keys' }); return; }
  const saved = accessControl.setAccess(req.params.userId, pages, actor ?? 'System Administrator');
