...
export async function recordBatch(params: {
  ...
  const components = await bom.getComponents(params.productItemId);

  const shortfalls = [];
  for (const c of components) {
    const needed = params.unitsActual * c.qtyPerUnit;
    const onHand = await inventory.getBalance(c.itemId);
    if (onHand < needed) shortfalls.push({ ...c, needed, onHand });
  }
  ...
  const id = await nextBusinessId('production_batches', 'PRD-', 4);
  await db.prepare(
  ...
  await activityLog.record(actor, 'recorded', 'production_batch', id, `Batch ${id}: ${params.unitsActual} units of ${params.productItemId}`);
...
export async function setStatus(id: string, status: string, actor = 'System Administrator'): Promise<void> {
  await db.prepare('UPDATE production_batches SET status = ? WHERE id = ?').run(status, id);
  await activityLog.record(actor, 'updated status of', 'production_batch', id, `Batch ${id} → ${status}`);
}
...
export async function reverseBatch(batchId: string, params: { reason: string; actor: string }): Promise<{ reversal: reversals.Reversal; batch: ProductionBatch }> {
  const batch = await getBatch(batchId);
  if (!batch) throw new Error(`Unknown production batch ${batchId}`);
  await reversals.assertNotReversed('production_batches', batchId);

  const packaged = (await db.prepare('SELECT COALESCE(SUM(quantity), 0) AS q FROM finished_goods WHERE batch_id = ?').get(batchId) as { q: number }).q;
  ...
  const components = await bom.getComponents(batch.product_item_id);
  ...
  return await db.transaction(async () => {
    ...
    return { reversal, batch: (await getBatch(batchId))! };
  });
}

export async function getBatch(id: string): Promise<ProductionBatch | undefined> {
  return await db.prepare('SELECT * FROM production_batches WHERE id = ?').get(id) as ProductionBatch | undefined;
}
...
export async function listBatches() {
  return await db.prepare(`
    SELECT pb.*, i.name AS product_name,
      ...
      GREATEST(pb.units_actual - (SELECT COALESCE(SUM(quantity), 0) FROM finished_goods WHERE batch_id = pb.id) - pb.rejected_quantity - pb.wasted_quantity, 0) AS not_yet_packaged,
      CASE
...
export async function closeBatch(batchId: string, params: { rejectedQuantity: number; wastedQuantity: number; actor: string }): Promise<ProductionBatch> {
  const batch = await getBatch(batchId);
  if (!batch) throw new Error(`Unknown production batch ${batchId}`);
  ...
  await db.prepare(`UPDATE production_batches SET rejected_quantity = ?, wasted_quantity = ?, closed_by = ?, closed_at = now() WHERE id = ?`)
...
export async function readyToPackage() {
  return await db.prepare(`
    SELECT pb.*, i.name AS product_name,
      (SELECT COALESCE(SUM(quantity), 0) FROM finished_goods WHERE batch_id = pb.id) AS packaged_units
