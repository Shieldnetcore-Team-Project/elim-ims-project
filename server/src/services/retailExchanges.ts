...
async function alreadyReturned(originalSalesId: string, itemId: string): Promise<number> {
...
export async function recordExchange(params: {
...
}): Promise<{ id: string; newSalesId: string | null }> {
  ...
  const order = await sales.getOrder(params.originalSalesId);
  ...
  const soldItems = new Map((await sales.listItemsFor(params.originalSalesId)).map(it => [it.item_id, it]));
  for (const line of lines) {
    const sold = soldItems.get(line.itemId);
    if (!sold) throw new Error(`${line.itemId} was not part of ${params.originalSalesId}`);
    const already = await alreadyReturned(params.originalSalesId, line.itemId);
  ...
  const id = await nextBusinessId('retail_exchanges', 'RXG-', 4);
  const returnedAmount = lines.reduce((sum, l) => sum + l.quantity * soldItems.get(l.itemId)!.unit_price, 0);

  return await db.transaction(async () => {
    await db.prepare('INSERT INTO retail_exchanges (id, original_sales_id, customer_id, reason, staff, actor) VALUES (?,?,?,?,?,?)')
    ...
    for (const line of lines) {
      ...
      await insertItem.run(id, line.itemId, line.quantity, unitPrice);
      await retailStock.reverseSale({ itemId: line.itemId, quantity: line.quantity, unitCost: unitPrice, salesId: params.originalSalesId, actor });
    }

    if (returnedAmount > 0) {
      await finance.postLedger({ account: 'Sales revenue', debit: returnedAmount, credit: 0, referenceType: 'retail_exchange', referenceId: id, description: `Return against ${params.originalSalesId} (${id})`, actor, customerId: order.customer_id ?? undefined });
      await finance.postLedger({ account: 'Cash/Bank', debit: 0, credit: returnedAmount, referenceType: 'retail_exchange', referenceId: id, description: `Refund for return against ${params.originalSalesId} (${id})`, actor });
    }

    let newSalesId: string | null = null;
    if (params.newSaleItems && params.newSaleItems.length > 0) {
      const newOrder = await sales.createOrder({
      ...
      await db.prepare('UPDATE retail_exchanges SET new_sales_id = ? WHERE id = ?').run(newSalesId, id);
    }
...
export async function getExchange(id: string): Promise<RetailExchange | undefined> {
  return await db.prepare('SELECT * FROM retail_exchanges WHERE id = ?').get(id) as RetailExchange | undefined;
}

export async function listItemsFor(exchangeId: string): Promise<RetailExchangeItem[]> {
  return await db.prepare(`
...
export async function listExchanges(): Promise<(RetailExchange & { customer_name: string })[]> {
  return await db.prepare(`
