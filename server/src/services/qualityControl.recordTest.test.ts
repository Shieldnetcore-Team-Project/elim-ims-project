...
async function makeBatch() {
  ...
  return await production.recordBatch({ productItemId: productId, line: 'Test Line', shift: 'Day', operator: 'Test Operator', unitsActual: 10 });
}

describe('qualityControl.recordTest (Section 19)', () => {
  it('passes only when every parameter passes, and explains why via the parameter breakdown', async () => {
    const batch = await makeBatch();
    const test = await qualityControl.recordTest({
    ...
    const detail = (await qualityControl.getTestDetail(test.id))!;
    expect(detail.parameters).toHaveLength(3);
    expect(detail.parameters.every(p => p.result === 'PASS')).toBe(true);
  });

  it('fails the whole test if any single parameter fails, even if others pass', async () => {
    const batch = await makeBatch();
    const test = await qualityControl.recordTest({
  ...
  it('never trusts a client-asserted PASS when a numeric range is given — recomputes independently', async () => {
    const batch = await makeBatch();
    const test = await qualityControl.recordTest({
      refType: 'PRODUCTION_BATCH', refId: batch.id, inspector: 'Test Inspector',
  ...
  it('trusts the tester\'s call for a qualitative parameter with no numeric range', async () => {
    const batch = await makeBatch();
    const test = await qualityControl.recordTest({
      refType: 'PRODUCTION_BATCH', refId: batch.id, inspector: 'Test Inspector',
  ...
  it('fails a batch on FAIL, same side effect as the legacy single-parameter recordResult', async () => {
    const batch = await makeBatch();
    await qualityControl.recordTest({
      refType: 'PRODUCTION_BATCH', refId: batch.id, inspector: 'Test Inspector',
      parameters: [{ name: 'pH', measuredValue: '2', minValue: 6.5, maxValue: 8.5, result: 'PASS' }],
    });
    expect((await production.getBatch(batch.id))!.status).toBe('FAILED');
  });

  it('reads the configured parameter list rather than a hard-coded pH-only set', async () => {
    await db.prepare(`
    ...
    `).run();
    expect(await qualityControl.parameterNames()).toEqual(['Chlorine', 'Iron', 'Total Hardness']);
