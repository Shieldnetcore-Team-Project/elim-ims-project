...
export async function listCategories(): Promise<string[]> {
  const row = await db.prepare(`SELECT value FROM settings WHERE id = 'Asset categories'`).get() as { value: string } | undefined;
...
export async function createAsset(params: {
  ...
  const id = await nextBusinessId('assets', 'AST-', 3);
  await db.prepare(`
  ...
  await activityLog.record(params.actor, 'registered asset', 'asset', id, `${id}: ${params.name}`);
  return (await getAsset(id))!;
}

export async function getAsset(id: string): Promise<Asset | undefined> {
  return await db.prepare('SELECT * FROM assets WHERE id = ?').get(id) as Asset | undefined;
}

export async function listAssets(): Promise<Asset[]> {
  return await db.prepare('SELECT * FROM assets ORDER BY id').all() as unknown as Asset[];
}
...
export async function updateAsset(id: string, patch: {
...
}, actor: string): Promise<Asset> {
  const existing = await getAsset(id);
  if (!existing) throw new Error(`Unknown asset ${id}`);
  await db.prepare('UPDATE assets SET location = ?, assigned_department = ?, service_interval_days = ?, notes = ?, status = ? WHERE id = ?').run(
  ...
  await activityLog.record(actor, 'updated asset', 'asset', id, `${id} updated`);
  return (await getAsset(id))!;
}
 ...
 *  entered twice. */
export async function recordService(id: string, params: { serviceDate: string; nextDue?: string | null }): Promise<void> {
  await db.prepare('UPDATE assets SET last_service = ?, next_due = COALESCE(?, next_due) WHERE id = ?').run(params.serviceDate, params.nextDue ?? null, id);
}
