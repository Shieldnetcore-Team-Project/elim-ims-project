...
async function resolveDepartment(actor: string): Promise<string | null> {
  const row = await db.prepare('SELECT role FROM users WHERE name = ?').get(actor) as { role: string | null } | undefined;
  return row?.role ?? null;
}

/** Every mutating service call ends with one of these — this table is the audit trail.
 *  `extra` (Module 17) is additive-only: every existing 5-arg call site keeps compiling
 *  unchanged and now gets ip/device/department captured for free; old_value/new_value/
 *  reason stay null unless a caller (a reversal, or a master-data edit) actually has
 *  something to report. */
export async function record(
  ...
  const department = extra?.department !== undefined ? extra.department : await resolveDepartment(actor);
  await db.prepare(
...
export async function list(limit = 200): Promise<ActivityEntry[]> {
  return await db.prepare(`SELECT * FROM activity_log ORDER BY id DESC LIMIT ?`).all(limit) as unknown as ActivityEntry[];
}
