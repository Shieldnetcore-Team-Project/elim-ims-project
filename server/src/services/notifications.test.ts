...
describe('notifications.allNotifications (Section 48 — notification center)', () => {
  it('surfaces an AWAITING_APPROVAL purchase order as a Pending Approval notification', async () => {
    ...
    const items = await notifications.allNotifications();
  ...
  it('surfaces an item at or below its reorder point as a Low Stock notification', async () => {
    const id = uniqueId('TST-ITEM-');
    await db.prepare(`INSERT INTO items (id, name, category, type, uom, reorder_point, unit_cost) VALUES (?,?,?,?,?,?,?)`)
  ...
  it('surfaces an overdue asset service as a Maintenance Due notification', async () => {
    const id = uniqueId('TST-ASSET-');
    await db.prepare(`INSERT INTO assets (id, equipment, next_due, status) VALUES (?,?, CURRENT_DATE - INTERVAL '2 days', 'ACTIVE')`).run(id, 'Test generator');
  ...
  it('surfaces a dispatched delivery run as a Pending Delivery notification', async () => {
  ...
  it('surfaces an AWAITING_APPROVAL payroll run as a Payroll Approval notification', async () => {
    ...
    await payroll.reviewRun(run.id, { reviewedBy: 'Test Chairman' });
  ...
  it('surfaces a completed production batch awaiting QC as a Quality Test Pending notification', async () => {
    const productId = await makeItem({ type: 'FINISHED_GOOD' });
    const batch = await production.recordBatch({ productItemId: productId, line: 'Test Line', shift: 'Day', operator: 'Test Operator', unitsActual: 10 });
  ...
  it('surfaces an expired vehicle document as a Document Expiry notification', async () => {
    const vehicleId = await makeVehicle();
    const doc = await vehicleDocuments.addDocument({ vehicleId, documentType: 'Insurance', expiryDate: '2020-01-01', actor: 'Test Officer' });
  ...
  it('reports Unclosed Till while the default till is open for today (isTillClosed itself is covered by tillClose.test.ts)', async () => {
    expect(await tillClose.isTillClosed()).toBe(false); // guards the premise below against run-order drift
    const items = await notifications.allNotifications();
