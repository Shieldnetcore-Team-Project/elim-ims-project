...
export function receiveGoods(params: {
  ...
  for (const it of params.items) {
    const expected = (expectedFor.get(params.poId, it.itemId) as { quantity: number } | undefined)?.quantity ?? null;
    insertItem.run(id, it.itemId, expected, procurement.resolveQuantity(it.itemId, it.quantity, it.bagQuantity));
  }
  procurement.setStatus(params.poId, 'RECEIVED', params.actor ?? params.receivedBy);
  activityLog.record(params.actor ?? params.receivedBy, 'recorded', 'goods_received', id, `Goods receipt ${id} against ${params.poId}, pending inspection`);
...
export function inspectGoodsReceived(grnId: string, params: {
  inspectionOfficer: string; lines: InspectionLine[]; actor?: string;
}): GoodsReceived {
  const grn = getGoodsReceived(grnId);
  ...
  const items = listItemsFor(grnId);
  ...
  const supplierId = (db.prepare('SELECT supplier_id FROM purchase_orders WHERE id = ?').get(grn.po_id) as { supplier_id: string }).supplier_id;
  ...
  try {
    ...
    db.prepare(`UPDATE goods_received SET status=?, inspection_officer=?, inspected_at=datetime('now') WHERE id=?`)
    ...
    if (rejectedLines.length > 0) {
      returnId = nextBusinessId('supplier_returns', 'SRN-', 4);
      db.prepare('INSERT INTO supplier_returns (id, grn_id, po_id, supplier_id, created_by) VALUES (?,?,?,?,?)')
      ...
      for (const l of rejectedLines) insertReturnItem.run(returnId, l.itemId, l.rejectedQuantity, l.rejectionReason ?? null);
    }
...
export function reverseGoodsReceived(grnId: string, params: { reason: string; actor: string }): { reversal: reversals.Reversal; grn: GoodsReceived } {
  const grn = getGoodsReceived(grnId);
  if (!grn) throw new Error(`Unknown goods receipt ${grnId}`);
  if (grn.status === 'PENDING_INSPECTION') throw new Error(`${grnId} hasn't been inspected yet — nothing to reverse`);
  reversals.assertNotReversed('goods_received', grnId);
  ...
  try {
    let acceptedValue = 0;
    for (const item of listItemsFor(grnId)) {
    ...
    if (acceptedValue > 0) {
      finance.postLedger({ account: 'Accounts payable', debit: acceptedValue, credit: 0, referenceType: 'goods_received_reversal', referenceId: grnId, description: `Reversal of goods accepted on ${grnId}`, actor: params.actor, supplierId });
      finance.postLedger({ account: 'Inventory', debit: 0, credit: acceptedValue, referenceType: 'goods_received_reversal', referenceId: grnId, description: `Reversal of goods accepted on ${grnId}`, actor: params.actor });
    }
    ...
    db.exec('COMMIT');
    return { reversal, grn: getGoodsReceived(grnId)! };
  } catch (err) {
    db.exec('ROLLBACK');
    throw err;
  }
