...
export function printCount(salesId: string): number {
  return (db.prepare(`SELECT COUNT(*) AS n FROM pos_receipt_prints WHERE sales_id = ? AND document_type = 'RECEIPT'`).get(salesId) as { n: number }).n;
}
...
export function recordPrint(params: { salesId: string; actor: string; documentType?: 'RECEIPT' | 'INVOICE'; overrideUserId?: string; reason?: string }): { isReprint: boolean } {
  ...
  if (documentType === 'INVOICE') {
    db.prepare(`INSERT INTO pos_receipt_prints (sales_id, is_reprint, printed_by, document_type) VALUES (?,0,?,'INVOICE')`)
      .run(params.salesId, params.actor);
    activityLog.record(params.actor, 'printed invoice for', 'sales', params.salesId, `${params.salesId} invoice printed`);
  ...
  db.prepare(`INSERT INTO pos_receipt_prints (sales_id, is_reprint, printed_by, approved_by, reason, document_type) VALUES (?,?,?,?,?,'RECEIPT')`)
