...
export function getComponents(productItemId: string): BomComponent[] {
  return db.prepare(`
...
export function setComponents(productItemId: string, components: { itemId: string; qtyPerUnit: number }[], actor: string): BomComponent[] {
  db.prepare('DELETE FROM bom_components WHERE product_item_id = ?').run(productItemId);
  const insert = db.prepare('INSERT INTO bom_components (product_item_id, component_item_id, qty_per_unit) VALUES (?,?,?)');
  for (const c of components) insert.run(productItemId, c.itemId, c.qtyPerUnit);
  activityLog.record(actor, 'updated recipe for', 'item', productItemId, `Recipe set to ${components.length} component(s)`);
  return getComponents(productItemId);
}
