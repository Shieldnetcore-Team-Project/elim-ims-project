...
export function listCategories(): string[] {
  const row = db.prepare(`SELECT value FROM settings WHERE id = 'Maintenance categories'`).get() as { value: string } | undefined;
  const fallback = [
...
function assertRefExists(refType: MaintenanceRefType, refId: string): void {
  const table = refType === 'ASSET' ? 'assets' : 'vehicles';
  const row = db.prepare(`SELECT 1 FROM ${table} WHERE id = ?`).get(refId);
...
export function recordExpense(params: {
  ...
  assertRefExists(params.refType, params.refId);
  const serviceDate = params.serviceDate ?? new Date().toISOString().slice(0, 10);

  const id = nextBusinessId('maintenance_records', 'MNT-', 4);
  db.exec('BEGIN');
  try {
    db.prepare(`
    ...
    if (params.refType === 'ASSET') {
      assets.recordService(params.refId, { serviceDate, nextDue: params.nextDueDate });
    }

    activityLog.record(params.actor, 'recorded maintenance for', params.refType.toLowerCase(), params.refId, `${id}: ${params.category} — ₦${params.amount.toLocaleString('en-NG')}`);
    db.exec('COMMIT');
  } catch (err) {
    db.exec('ROLLBACK');
    throw err;
...
export function getRecord(id: string): MaintenanceRecord | undefined {
  return db.prepare('SELECT * FROM maintenance_records WHERE id = ?').get(id) as MaintenanceRecord | undefined;
}

export function listRecords(filter?: { refType?: MaintenanceRefType; refId?: string }): MaintenanceRecord[] {
  if (filter?.refType && filter?.refId) {
    return db.prepare('SELECT * FROM maintenance_records WHERE ref_type = ? AND ref_id = ? ORDER BY id DESC').all(filter.refType, filter.refId) as unknown as MaintenanceRecord[];
  }
  if (filter?.refId) return db.prepare('SELECT * FROM maintenance_records WHERE ref_id = ? ORDER BY id DESC').all(filter.refId) as unknown as MaintenanceRecord[];
  if (filter?.refType) return db.prepare('SELECT * FROM maintenance_records WHERE ref_type = ? ORDER BY id DESC').all(filter.refType) as unknown as MaintenanceRecord[];
  return db.prepare('SELECT * FROM maintenance_records ORDER BY id DESC').all() as unknown as MaintenanceRecord[];
}
