...
export async function listSuppliers(): Promise<Supplier[]> {
  return await db.prepare('SELECT * FROM suppliers ORDER BY name').all() as unknown as Supplier[];
}
...
export async function createSupplier(s: Supplier): Promise<void> {
  await db.prepare('INSERT INTO suppliers (id, name, location) VALUES (?,?,?)').run(s.id, s.name, s.location);
}
...
export async function resolveQuantity(itemId: string, quantity: number, bagQuantity: number | undefined): Promise<number> {
  if (bagQuantity == null) return quantity;
  const item = await db.prepare('SELECT pieces_per_bag FROM items WHERE id = ?').get(itemId) as { pieces_per_bag: number | null } | undefined;
...
export async function createPurchaseOrder(params: {
  ...
  const id = await nextBusinessId('purchase_orders', 'PO-2026-', 5);
  await db.prepare('INSERT INTO purchase_orders (id, supplier_id, requested_by, requested_by_user_id, status) VALUES (?,?,?,?,?)')
  ...
  for (const it of params.items) {
    const quantity = await resolveQuantity(it.itemId, it.quantity, it.bagQuantity);
    await insertItem.run(id, it.itemId, quantity, it.unitPrice);
  }
  await activityLog.record(params.actor ?? params.requestedBy, 'created', 'purchase_order', id, `Purchase order ${id}, ${params.items.length} item line(s)`);
...
export async function adjustLinePrice(poId: string, itemId: string, newUnitPrice: number, params: { reason: string; actor: string }): Promise<PurchaseOrder> {
  ...
  const line = await db.prepare('SELECT id, unit_price FROM purchase_order_items WHERE po_id = ? AND item_id = ?').get(poId, itemId) as { id: number; unit_price: number } | undefined;
  if (!line) throw new Error(`${itemId} is not a line on ${poId}`);

  await db.prepare('UPDATE purchase_order_items SET unit_price = ? WHERE id = ?').run(newUnitPrice, line.id);
...
export async function setStatus(id: string, status: string, actor = 'System Administrator'): Promise<PurchaseOrder | null> {
  const res = await db.prepare('UPDATE purchase_orders SET status = ? WHERE id = ?').run(status, id);
  if (res.changes === 0) return null;
  await activityLog.record(actor, 'updated status of', 'purchase_order', id, `Purchase order ${id} → ${status}`);
  return (await getPurchaseOrder(id)) ?? null;
}

export async function getPurchaseOrder(id: string): Promise<PurchaseOrder | undefined> {
  return await db.prepare('SELECT * FROM purchase_orders WHERE id = ?').get(id) as PurchaseOrder | undefined;
}

export async function listPurchaseOrderItems(poId: string): Promise<PurchaseOrderItem[]> {
  return await db.prepare('SELECT * FROM purchase_order_items WHERE po_id = ?').all(poId) as unknown as PurchaseOrderItem[];
}
