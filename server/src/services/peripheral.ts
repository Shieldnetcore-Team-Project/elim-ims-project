...
export function employeeStatusOptions(): { value: string; label: string }[] {
  const row = db.prepare(`SELECT value FROM settings WHERE id = 'Employee status options'`).get() as { value: string } | undefined;
