-- ============================================================================
-- FIX: function public.has_permission(uuid, unknown, unknown) is not unique
-- ----------------------------------------------------------------------------
-- 20260815094000_action_permission_and_status_engine.sql added a second
-- has_permission(uuid, module_key, action_key) overload alongside the
-- original has_permission(uuid, module_key, access_level) without ever
-- dropping the old one. RLS policies bind to a specific overload by OID at
-- CREATE POLICY time, so they kept working -- but the SQL inside a plpgsql
-- function body isn't resolved until first EXECUTION, so every RPC below
-- sat as a dormant landmine until a user actually exercised that code path
-- (confirmed live on Sales: create_sale -> "not unique").
--
-- Fix: CREATE OR REPLACE every live, reachable RPC below with the exact
-- same signature, adding an explicit ::action_key cast to the literal so
-- Postgres can pick the right overload. No signature changes, so this
-- carries zero CASCADE risk to anything (in particular the many RLS
-- policies in 20260814093000_rls_permission_rewrite.sql that are correctly
-- bound to the old access_level overload by OID and are not part of this
-- bug). The old overload is deliberately NOT dropped for the same reason.
--
-- NOTE (superseded): this cast alone turned out to be insufficient -- see
-- 20260816109000_fix_has_permission_module_cast.sql, which supersedes the
-- 13 functions below with an additional ::module_key cast. Left as-is here
-- for an accurate history of what actually ran on the remote at this step.
--
-- Scope: every function below is verified (by cross-referencing every
-- `has_permission(..., 'literal')` call-site against a full map of each
-- function's latest CREATE OR REPLACE across all migrations) to be both
-- (a) the current live definition and (b) not EXECUTE-revoked from
-- authenticated. Functions matching the pattern but already superseded by a
-- later, already-cast definition, or already REVOKEd from authenticated
-- (receive_raw_material, adjust_raw_material, adjust_finished_stock,
-- delete_production), are excluded -- they are unreachable or already safe.
-- ============================================================================
...
DECLARE
  ...
  v_price numeric;
BEGIN
  IF NOT public.has_permission(v_uid, 'sales', 'write'::action_key) THEN RAISE EXCEPTION 'Insufficient permissions'; END IF;
...
BEGIN
  IF NOT (public.has_permission(v_uid, 'payments', 'write'::action_key) OR public.has_permission(v_uid, 'debts', 'write'::action_key)) THEN
...
DECLARE
  ...
  v_id uuid;
BEGIN
  IF NOT public.has_permission(v_uid, 'receipts-payments', 'write'::action_key) THEN RAISE EXCEPTION 'Insufficient permissions'; END IF;
  IF v_factory IS NULL THEN RAISE EXCEPTION 'factory_id required'; END IF;
...
BEGIN
  IF NOT public.has_permission(v_uid, 'raw-materials', 'write'::action_key) THEN RAISE EXCEPTION 'Insufficient permissions'; END IF;
...
DECLARE
  ...
  v_to_name text;
BEGIN
  IF NOT public.has_permission(v_uid, 'finished-goods', 'write'::action_key) THEN RAISE EXCEPTION 'Insufficient permissions'; END IF;
  IF v_qty IS NULL OR v_qty <= 0 THEN RAISE EXCEPTION 'Quantity must be > 0'; END IF;
...
DECLARE
  ...
  v_role text;
BEGIN
  IF NOT public.has_permission(v_uid, 'account-approvals', 'write'::action_key) THEN RAISE EXCEPTION 'Insufficient permissions'; END IF;
...
DECLARE
  ...
  v_id uuid;
BEGIN
  IF NOT public.has_permission(v_uid, 'logistics', 'write'::action_key) THEN
    RAISE EXCEPTION 'Insufficient permissions';
  END IF;
  IF v_factory IS NULL THEN RAISE EXCEPTION 'factory_id required'; END IF;
