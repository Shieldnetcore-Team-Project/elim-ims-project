-- ============================================================================
-- Let Production record damages at batch-creation time, not just Store at
-- confirmation
-- ----------------------------------------------------------------------------
-- production.damaged_quantity already existed but was only ever written by
-- confirm_production_batch() (Store, required to be a different person than
-- whoever logged the batch — a deliberate segregation-of-duties check). Staff
-- on the shop floor (both Water and Nylon — production.tsx is shared across
-- both factories, isolated only by production_scope) want to log damage they
-- already know about the moment they record the run, instead of waiting for
-- Store to (re-)discover and re-enter it. This is an intentional relaxation
-- of that control for the creation-time figure specifically — Store's own
-- confirm step, and its same-person restriction, are untouched.
--
-- ConfirmBatchDialog (Finished Goods) is updated separately (frontend-only)
-- to default its "Damaged Quantity" field from this value instead of 0, so
-- Store sees and can adjust what Production already reported rather than
-- silently overwriting it with zero.
-- ============================================================================
...
DECLARE
  ...
  v_damaged numeric := COALESCE((payload->>'damaged_quantity')::numeric, 0);
...
BEGIN
  ...
  IF v_damaged < 0 THEN RAISE EXCEPTION 'Damaged quantity cannot be negative'; END IF;
  IF v_damaged > v_qty THEN RAISE EXCEPTION 'Damaged quantity cannot exceed quantity produced'; END IF;
